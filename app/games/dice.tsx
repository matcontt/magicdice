        ...
        <View className="flex-1 px-6 py-4">
            ...
            <View className="bg-white/5 rounded-2xl p-5 border border-white/10">
              ...
              <View className="flex-row justify-between items-center pt-4 border-t border-white/10">
                <View className="flex-row items-center space-x-2">
                  ...
                  <Text className={`text-xs font-medium ${isActive ? 'text-green-400' : 'text-red-400'}`}>
                    {isActive ? 'Sensor Active' : 'Sensor Inactive'}
                  </Text>
                </View>
                <View className="flex-row items-center space-x-2">
                  <Text className="text-gray-500 text-xs">Force:</Text>
                  <Text className={`text-xs font-mono font-bold ${magnitude > 1.78 ? 'text-pink-400' : 'text-gray-400'}`}>
          ...
          <Animated.View entering={FadeInUp.delay(400)} className="space-y-3">
            <Pressable onPress={handleDiceRoll} disabled={isRolling} className="active:scale-95">
              <View className={`bg-gradient-to-r from-violet-600 via-purple-600 to-pink-600 rounded-2xl py-5 ${
            ...
            {rollCount > 0 && (
              <Pressable onPress={handleReset} className="active:scale-95">
                <View className="bg-white/5 border border-white/10 rounded-xl py-4">
                  <Text className="text-gray-400 text-sm font-semibold text-center">Reset Counter</Text>
                </View>
              </Pressable>
            )}
          </Animated.View>
